...
def _reindex_all() -> None:
    """Serialized worker used by uploads/replacements; never blocks API handlers."""
    with _index_lock:
...
def _document_rows() -> list[DocumentMetadata]:
    ...
    for standard_id, (metadata, chunk_count) in sorted(counts.items()):
        rows.append(DocumentMetadata(
            source_file=str(metadata.get("source_file", metadata.get("source", ""))), standard_id=standard_id,
            year=str(metadata.get("year", "")), document_type=str(metadata.get("document_type", "")),
            title=str(metadata.get("title", "")), source_url=str(metadata.get("source_url", "")), chunk_count=chunk_count,
        ))
    return rows
...
async def create_document(background_tasks: BackgroundTasks, file: UploadFile = File(...)) -> IndexJob:
    ...
    if target.exists():
        raise HTTPException(status_code=409, detail="A document with this filename already exists; use PATCH to replace it.")
    ...
    return IndexJob(status="queued", source_file=filename, detail="Saved; background indexing has started.")
...
async def delete_document(standard_id: str) -> dict[str, str | int]:
    """Remove every vector chunk for one canonical standard; raw files are retained."""
